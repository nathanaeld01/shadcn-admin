...
>(({ children, className, defaultOpen = true, style, ...props }, ref) => {
	const sidebarStore = React.useRef(useSidebarStore({ defaultOpen })).current();
	const contextValue = React.useMemo(() => sidebarStore, [sidebarStore]);

	useKey(`ctrl + ${SIDEBAR_KEYBOARD_SHORTCUT}`, {
		preventDefault: true,
		onKeyDown: () => contextValue.toggle(),
	});
...
>(({ children, className, collapsible = "offcanvas", side = "left", variant = "sidebar", ...props }, ref) => {
	const { isMobile, isOpenMobile, state, toggle } = useSidebar();
	...
	if (isMobile) {
		return (
			<Sheet onOpenChange={toggle} open={isOpenMobile} {...props}>
				<SheetContent
	...
	return (
		<aside
			className={cn(
				"group peer hidden w-sidebar bg-sidebar text-sidebar-foreground transition-[width] duration-300 md:block",
				"shrink-0 overflow-hidden data-[state=collapsed]:data-[collapsible=offcanvas]:w-0",
			)}
		...
		>
			<div
				className={cn(
					"relative inset-y-0 z-10 hidden h-svh w-sidebar flex-col border-sidebar-border bg-sidebar transition-[left,right,width] duration-200 ease-linear group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow md:flex",
					side === "left"
